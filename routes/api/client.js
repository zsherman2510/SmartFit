...
router.get('/client/:client_id', (req, res) => {
	...
	Client.findOne(req.params.client_id)
		...
		.catch((err) => res.status(404).json({ client: 'There is no client for this client.' }));
});
...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
	...
	Client.findOne(req.client.id).then((client) => {
		if (client) {
			//Update
			Client.findOneAndUpdate({ client: req.client.id }, { $set: clientFields }, { new: true }).then((client) =>
				res.json(client)
			);
		} else {
			//Create
